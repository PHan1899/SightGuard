...
def get_accelerate_model(
    ...
    if checkpoint_dir is not None and checkpoint_dir !="None":
    ...
    if args.base_model_name in REGISTERED_BASE_MODELS and reuse_base_model:
        config = {
        ...
        if registered_config == config and not args.full_finetune:
            print(f"loading registered model {args.base_model_name}...")
            ...
            raise ValueError(
                f"Model {args.base_model_name} is already registered with a different config."
    ...
    if not args.full_finetune:
        ...
        if (checkpoint_dir is not None) and (checkpoint_dir !="None"):
            ...
            if args.base_model_name not in REGISTERED_BASE_MODELS:
                config = {
                    "load_in_4bit": args.bits == 4,
                    "load_in_8bit": args.bits == 8,
                    "llm_int8_threshold": 6.0,
                ...
                REGISTERED_BASE_MODELS[args.base_model_name] = (model, config)
...
def get_peft_model(model, peft_config, adapter_name="default"):
    ...
    """
    peft_config.base_base_model_name = model.__dict__.get("name_or_path", None)
